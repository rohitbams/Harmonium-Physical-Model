...
//        DBG("Processing with frequency: " + juce::String(currentFrequency_));
    ...
    void setAmplitudeScale(int amp) {
        physicsConfig.amplitudeScale = amp;
    }

//    void setLows(int value) {
//        physicsConfig.low = value;
//    }
    
    void setMu(double mu) {
        physicsConfig.mu = std::clamp(mu, 1e-6, 1e-1);
    }
...
//        }
//        return totalAmplitude / reedInstances.size();
//    }
...
private:
    void createReedsForMode(double frequency) {
        ...
        switch (reedMode_) {
            ...
            case DOUBLE_REED:
                reedInstances.emplace_back(0.0);   // Fundamental
                reedInstances.emplace_back(-12.1); // One octave lower
                break;
                    
            case TRIPLE_REED:
                reedInstances.emplace_back(-12.5); // Slightly flat octave
                reedInstances.emplace_back(0.0);   // Fundamental
                reedInstances.emplace_back(+12.2); // Slightly sharp octave
                break;
        }
        
        DBG("Created " + juce::String(reedInstances.size()) + " reed instances");
    }
};
